...
namespace NoteAppBackEnd.Repositories
{
    public class NoteRepository
    {
        ...
        public int Insert(Note note)
        {
            ...
            command.Parameters.AddWithValue("@LocationOfProject", note.LocationOfProject);
            command.Parameters.AddWithValue("@Created_by", note.Created_by ?? (object)DBNull.Value);

            int noteId = Convert.ToInt32(command.ExecuteScalar());
            return noteId;
        }
        ...
        private Note MapNote(SqlDataReader reader)
        {
            ...
            return new Note
            {
                ...
                LocationOfProject = reader["Location_of_Project"] as string,
        ...
        public List<Note> GetAllNotePaged(int pageNumber, int pageSize, string sortColumn, string sortDirection, string userId, string? search)
        {
            var cmd = _connection.CreateCommand();
            cmd.Transaction = _transaction;
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "uspGetPageAllNote";
